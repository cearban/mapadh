...
def render_2_5D_scene(min_x,min_y,max_x,max_y,overlay_type,look_dir,scene_name):
    working_dir = 'D:\\tmp\\'
    ...
    wcs_request_str = 'http://129.215.169.69:666/cgi-bin/mapserv.exe?map=os_profile_dtm_wcs_c.map&SERVICE=WCS&VERSION=1.0.0&REQUEST=GetCoverage&COVERAGE=os_dem&CRS=EPSG:27700&BBOX=' + aoi + '&WIDTH=500&HEIGHT=500&FORMAT=image/tif'
    ...
    while 1:
        line = p.readline()
        if not line: break
        found_min = line.find('STATISTICS_MINIMUM')
        found_max = line.find('STATISTICS_MAXIMUM')
        if found_min > 0:
            tokens = line.split('=')            
            min_z = float(tokens[1]) 
        if found_max > 0:
            tokens = line.split('=')            
            max_z = float(tokens[1]) 

    while 1:
        if found_max & found_min: break
    ...
    wrote_scene = time.time()
    sceneinterval = wrote_scene - gdal_complete
    
    # run povray to render the scene
    
    cmd = '"D:/POV-Ray for Windows v3.62/bin/pvengine.exe" /render ' + working_dir + scene_def_filename + ' +A +W800 +H800 +O -GA +Q10 -D' + working_dir + output_scene_filename + ' /exit'
    os.system(cmd)
...
def main():
    if len(sys.argv) == 8:
        ...
        render_2_5D_scene(int(min_x),int(min_y),int(max_x),int(max_y),overlay_type,look_dir,scene_name)
    else:
